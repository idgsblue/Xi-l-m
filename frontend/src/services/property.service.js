...
const propertyService = {
  ...
  // ✅ CORREGIDO: Obtener mis propiedades (host)
  ...
  // ✅ NUEVO: Anunciar propiedad (cambiar de approved a published)
  advertiseProperty: async (propertyId) => {
    try {
      const response = await api.post(`/properties/${propertyId}/advertise`);
      return response.data;
    } catch (error) {
      throw error.response?.data || { message: 'Error al anunciar propiedad' };
    }
  },

  // ✅ NUEVO: Despublicar propiedad (cambiar de published a approved)
  unadvertiseProperty: async (propertyId) => {
    try {
      const response = await api.post(`/properties/${propertyId}/unadvertise`);
      return response.data;
    } catch (error) {
      throw error.response?.data || { message: 'Error al despublicar propiedad' };
    }
  },
